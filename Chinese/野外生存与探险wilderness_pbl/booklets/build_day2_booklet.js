// Build day1_booklet.docx — Wilderness Unit · Day 2: 营地搭建
...
const coverChildren = [
  ...
  new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { before: 200, after: 100 },
    children: [new TextRun({ text: 'Day 2 · 营地搭建', bold: true, size: 44, color: DARK })],
  }),
  new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { before: 100, after: 800 },
    children: [new TextRun({ text: 'Camp Setup', italics: true, size: 28, color: GRAY })],
  }),
  new Paragraph({
    spacing: { before: 1200, after: 200 },
    children: [
...
const mcQuestions = [
  ...
  {
    "img": "在哪里搭帐篷最安全？  Best place for a tent?",
    "q": "最安全的位置？  Safest location?",
    "options": [
      "河边湿地 Wet riverside",
      "平坦草地 Flat grassland",
      "山顶悬崖 Mountain top cliff"
    ]
  },
  {
    "img": "篝火 — 跳跃的火焰  Camp fire",
    "q": "在火附近, 我们应该？  Near the fire we should…",
    "options": [
      "远离 Stay back",
      "靠近用手碰 Touch it",
      "上面跳过去 Jump over it"
  ...
  {
    "img": "背包 — 装好的背包  Packed backpack",
    "q": "背包里要带什么？  What should be in the bag?",
    "options": [
      "玩具和糖果 Toys & candy",
      "水, 食物, 急救 Water, food, first-aid",
      "电视和游戏机 TV & games"
    ]
  }
];
...
const section2Children = [
  new Paragraph({ pageBreakBefore: true, spacing: { before: 0, after: 0 }, children: [new TextRun({ text: '' })] }),
  shadedBar('二、森林露营要带什么？ / Pack for Forest Camping', SUN, 22),
  new Paragraph({
    spacing: { before: 100, after: 60 },
    children: [new TextRun({ text: '👉 如果你要去森林露营, 你的背包里要带什么？写一写, 画一画。', size: 22, bold: true, color: DARK })],
  }),
  new Paragraph({
    spacing: { before: 40, after: 100 },
    children: [new TextRun({ text: 'If you are going forest camping, what do you pack in your backpack? Write and draw.', size: 16, italics: true, color: GRAY })],
  }),
];
...
section2Children.push(new Paragraph({
  ...
  children: [
    new TextRun({ text: '🎨 写一写 / 画一画  Write + Draw your backpack ', size: 22, bold: true, color: SUN }),
    new TextRun({ text: '— 把你要带的东西画下来或写下来！', size: 14, italics: true, color: GRAY }),
  ],
}));
...
const section4Children = [
  new Paragraph({ pageBreakBefore: true, spacing: { before: 0, after: 0 }, children: [new TextRun({ text: '' })] }),
  shadedBar('四、描一描, 写一写 / Trace and Write  (火  包  水)', ALERT, 24),
  new Paragraph({
    spacing: { before: 200, after: 100 },
    children: [new TextRun({
      text: '👉 在下面贴上写字纸, 写一写今天学到的字: 火  包  水',
      size: 22, italics: true, color: GRAY,
    })],
  }),
  new Paragraph({
