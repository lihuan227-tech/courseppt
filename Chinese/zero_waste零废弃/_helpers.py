...
def teacher_box(s, l, t, w, say_cn, do_cn, mins, color=None):
    ...
    tb(s, l + 0.15, t + 0.06, 1.40, 0.22, "👩‍🏫 老师 说",
       sz=9, b=True, c=color)
    tb(s, l + 0.15, t + 0.28, w - 1.15, 0.30, say_cn,
       sz=11, b=True, c=DARK)
...
def vocab_write(prs, header_color, cn_phrase, en_word, chars):
    ...
    csz = 120 if len(cn_phrase) == 1 else 88
...
OCEAN_TINTS = [RGBColor(0x1B,0x6F,0xBA), RGBColor(0x00,0x69,0x70), RGBColor(0x2A,0x9D,0x8F),
               RGBColor(0x3A,0x7C,0xA5), RGBColor(0xC0,0x5A,0x17), RGBColor(0xC8,0x25,0x3E)]
...
def observe_think_slide(prs, color, header, photo_cn, photo_en, senses, dangers,
    ...
    s = ns(prs); bg(s, CREAM); hb(s, header, color)
    photo_slot(s, 0.30, 0.95, 4.35, 3.05, photo_cn, photo_en, color)
    panel(s, 4.80, 0.95, 4.90, 3.05, color, fill=WHITE, lw=2.5)
    ...
    if media_cn and media_url:
        link = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(4.90), Inches(3.32), Inches(4.70), Inches(0.55))
        link.fill.solid(); link.fill.fore_color.rgb = INK; link.line.color.rgb = STAR; link.line.width = Pt(1.5)
        tb(s, 4.95, 3.36, 4.6, 0.24, f"▶️ {media_cn}", sz=11, b=True, c=STAR, a=PP_ALIGN.CENTER)
        tb(s, 4.95, 3.60, 4.6, 0.22, media_url, sz=7.5, c=LGRAY, a=PP_ALIGN.CENTER)
        link.click_action.hyperlink.address = media_url
    dh = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.30), Inches(4.12), Inches(9.40), Inches(0.38))
    dh.fill.solid(); dh.fill.fore_color.rgb = ALERT; dh.line.fill.background()
    tb(s, 0.4, 4.17, 9.2, 0.30, "2️⃣ 🤔 想 — 它 到 了 海 里, 会 伤 害 谁?  我 觉 得……",
       sz=12, b=True, c=STAR, a=PP_ALIGN.CENTER)
    dw, dgap = 2.28, 0.10; dx0 = (10 - (4*dw+3*dgap))/2
    for i, (em, cn) in enumerate(dangers[:4]):
        x = dx0 + i*(dw+dgap)
        panel(s, x, 4.60, dw, 0.60, color, fill=WHITE, lw=2)
        tb(s, x+0.10, 4.69, 0.5, 0.42, em, sz=20, a=PP_ALIGN.LEFT)
        tb(s, x+0.62, 4.71, dw-0.7, 0.40, cn, sz=12, b=True, c=DARK, a=PP_ALIGN.LEFT)
    return s


def _ab_rows(s, color, rows, reveal=False, top=1.35):
    ...
    for i, (sc, a_cn, b_cn, correct) in enumerate(rows[:3]):
        ...
        for label, txt, bx in (("A", a_cn, 4.10), ("B", b_cn, 6.90)):
            ...
            tb(s, bx+0.10, y+0.20, 0.60, 0.35, ("✓ "+label if good else label), sz=12, b=True, c=oc, a=PP_ALIGN.LEFT)
...
def judge_ab_slide(prs, color, header, rows, frame_cn, frame_en):
    ...
    vb = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.40), Inches(3.98), Inches(9.20), Inches(0.40))
    vb.fill.solid(); vb.fill.fore_color.rgb = color; vb.line.fill.background()
    tb(s, 0.5, 4.03, 9.0, 0.30, "👉 A → 举 左 手 · B → 举 右 手   🤔 3… 2… 1…",
       sz=12, b=True, c=STAR, a=PP_ALIGN.CENTER)
    sentence_frame_bar(s, 4.58, frame_cn, frame_en, color)
    return s
...
def reveal_ab_slide(prs, color, header, rows, do_action, frame_cn, frame_en):
    ...
    ab = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.40), Inches(3.98), Inches(9.20), Inches(0.40))
    ab.fill.solid(); ab.fill.fore_color.rgb = OK; ab.line.fill.background()
    tb(s, 0.5, 4.02, 9.0, 0.32, f"🙆 做: {do_action}", sz=12, b=True, c=WHITE, a=PP_ALIGN.CENTER)
    sentence_frame_bar(s, 4.58, frame_cn, frame_en, color)
    return s
...
def cardgrid_slide(prs, color, header, subtitle, items):
    ...
    cw, ch, gx, gy = 2.90, 1.75, 0.22, 0.18
    x0 = (10 - (3*cw + 2*gx))/2; y0 = 1.28
...
def guess_slide(prs, color, header, subtitle, cards, frame_cn, frame_en):
    ...
    cw, ch, gx, gy = 2.90, 1.45, 0.22, 0.22
    x0 = (10 - (3*cw + 2*gx))/2; y0 = 1.35
